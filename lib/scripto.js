...
function loadScriptsFromDir(scriptsDir) {
    ...
    var getFileContent = function(filename) {
        var lines = fs.readFileSync(filename, 'utf8').match(/[^\r\n]+/g).map((line) => {
            var match = line.match(/^[\s]*scripto_include[\s]*[(]["']([^"']+)['"][\s]*[)][\s]*$/);
            if (match) {
                return '\n---- include ' + match[1] + ' ----\n\n' +
                    getFileContent(path.dirname(filename) + '/' + match[1]) +
                    '\n\n---- end include ' + match[1] + ' ----\n\n';
            }
            else {
                return line;
    ...
    names.forEach(function(name) {
        ...
        if (name.endsWith('.lua')) {
            ...
            scripts[key] = getFileContent(filename);
        }
    });
